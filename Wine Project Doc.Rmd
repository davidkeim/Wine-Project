...
## Executive Summary
For my capstone choose your own project, I wanted to find something of personal interest and I stumbled across a wine data on UCI Marchine Learning Archive. I’ve always been under the impression that there’s some je ne sais quoi about wine quality and I think it will be fascinating to some analysis on. We will see if we can predict the assigned quality rating on our validation set based on inputs from from our training set.
## Analysis and Methods
(explains the process and techniques used, such as data cleaning, data exploration and visualization, any insights gained, and your modeling approach)
...
When we initially plot our quality rankings we identify our first problem. Over 93% of wines are assigned a ranking of a 5, 6, or 7. While this will make predicting a score close to the actual score relatively easy, thinking as a sommelier, we don’t want to miss those wonderful 8’s and 9’s or risk giving someone a 3 or a 4.
...
A few variables do look promising: alcohol (higher values seem to lead to an improved quality), density (lower points to a higer quality. My experience as a homebrewer tells me that alcohol levels have an effect on density), chloride, and volatile acids as well. 
...
We want to throw a bunch of different models at this challenge to see which ones are effective but before we do, we need to trim down to the most useful factors. A smaller set of factors will allow us to try more models without taking hours. We run a Recursive Feature Elimination assessment to identify the top 5 factors by importance and get the result of alcohol, volatile.acidity, free.sulfur.dioxide, sulphates, chlorides. We already identified a couple of these in through our initial visualisations.
...
Now that we have a smaller set of factors lets use some models on them to see how they perform. This does take some time to run. We see a couple models over 60% accuracy, a second group around 50%, and gamLoess just lost out in space. The top was Random Forest with an accuracy of 67.7%.

![figure 14](figure14.png)

Our coursework has taught that combining multiple models should lead to an improvement in the accuracy. We'll combine our five models and see if we can improve on their individual performance. For this to be effective we want to compare correlation between models. If they're overcorrelated (>.75), then we wont see improvement. We can see that we don't have any worries for this set.

![figure 15](figure15.png)

I tried to use the caretEnsemble package to accomplish this but it threw me an error with multiclass problems. We'll have to build our ensemble in a similar manner to how we built the first one determing 2s against 7s with the MNIST dataset in the course. We will add back the additional factors that we trimmed earlier to bump up the accuracy if we can. We get an accuracy of 70.2% which is a rather small improvement. Becuase this improvement is so small, I'm going to go back and see if we can't adjust any of the tuning parameters on the individual models to get an improvement. The challenge is that the tree based models already take a bit of time to run.
